...
#include <ros/ros.h>
//#include "gazebo_ros/node.h"
#include "geometry_msgs/Twist.h"
#include "tello_msgs/FlightData.h"
#include "tello_msgs/TelloResponse.h"
#include "tello_msgs/TelloAction.h"
...
class TelloPlugin : public gazebo::ModelPlugin
{
  ...
  // ROS publishers
  ros::Publisher flight_data_pub_;// = node_.advertise<tello_msgs::FlightData> ;
  ros::Publisher tello_response_pub_;// = node_.advertise<tello_msgs::TelloResponse >::SharedPtr tello_response_pub_;

  // ROS services
  ros::Subscriber/*Service<tello_msgs::srv::TelloAction>::SharedPtr*/ command_srv_;
...
public:
  ...
  void set_target_velocities(std::string rc_command)
  {
    ...
    try {
      ...
      ROS_ERROR("can't parse rc command '%s', exception %s", rc_command.c_str(), e.what());
      return;
    }
  ...
  {
    //if (node_ != nullptr) {
      ROS_INFO("transition from '%s' to '%s'", state_strs_[flight_state_], state_strs_[next]);
    //}
  ...
  void Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
  {
    ...
    bool use_sim_time;
    node_.getParam("use_sim_time", use_sim_time);
    if (!use_sim_time) {
      ROS_ERROR("use_sim_time is false, could be a bug");
    }

    // ROS publishers
    flight_data_pub_ = node_.advertise<tello_msgs::FlightData>("flight_data", 1);
    tello_response_pub_ = node_.advertise<tello_msgs::TelloResponse>("tello_response", 1);

    // ROS service
    command_srv_ = node_.subscribe("tello_action", 10,
      &TelloPlugin::command_callback, this);
	/**/
    // ROS subscription
    cmd_vel_sub_ = node_.subscribe("cmd_vel", 10, &TelloPlugin::cmd_vel_callback, this);
...
#pragma ide diagnostic ignored "UnusedValue"
  void command_callback(
    ...
    const tello_msgs::TelloAction request)//,
    //std::shared_ptr<tello_msgs::srv::TelloAction::Response> response)
  {
    if (request.cmd == "takeoff" && flight_state_ == FlightState::landed) {
      ...
      ROS_WARN("ignoring command '%s'", request.cmd.c_str());
...
#pragma clang diagnostic pop
  ...
  void spin_10Hz()
  {
    ...
    // Simulate a battery
    int battery_percent = static_cast<int>((battery_duration_ - ros_time.sec) / battery_duration_ * 100);
    ...
    // Publish flight data
    tello_msgs::FlightData flight_data;
